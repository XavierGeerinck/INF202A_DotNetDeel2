...
namespace Server
{
    class ListenerSocket
    {
        ...
        public ListenerSocket()
        {
            ...
            // Ask for the ip of the loadbalancer
            Console.Write("Enter the loadbalancer ip: "); // Prompt
            loadBalancerIp = IPAddress.Parse(Console.ReadLine()); // Get string from user

            // Connect to the loadbalancer
            Logger.ShowMessage(String.Format("Connecting to loadbalancer on: {0}:{1}", loadBalancerIp.ToString(), int.Parse(Server.Properties.Resources.LoadBalancerPort)));
            loadBalancerSocket.Connect(loadBalancerIp, int.Parse(Server.Properties.Resources.LoadBalancerPort));
            Logger.ShowMessage("Connected to the loadbalancer.");
